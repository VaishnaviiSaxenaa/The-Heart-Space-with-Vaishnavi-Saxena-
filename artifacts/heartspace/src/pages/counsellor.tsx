    ...
    <div className="space-y-3">
      {[
        ...
        <div key={key} className="flex items-center gap-3">
          <span className="text-xs w-32" style={{ color: MUTED }}>
            {label}
          </span>
          <input
            type="number"
      ...
      <p className="text-xs" style={{ color: OLIVE }}>
        Total: {hrs * days} hrs/week · {hrs * days * 4 * months} hrs over{" "}
        {months} months
      </p>
    </div>
  );
    ...
    <div className="space-y-2">
      {JAM_SUBJECTS.map((subject) => {
        ...
        const done = topics.filter(
          (t: unknown) => (t as Record<string, unknown>)?.status === "done",
        ).length;
        ...
        return (
          ...
          >
            ...
            <span className="text-xs w-8 text-right" style={{ color: MUTED }}>
              {pct}%
            </span>
          </div>
        );
        ...
        return (
          ...
          >
            ...
            <span
              ...
              style={{
                background: acc >= 80 ? `${OLIVE}22` : `${GOLD}22`,
                color: acc >= 80 ? OLIVE : GOLD,
              }}
            >
              {totalQ > 0 ? `${acc}%` : "—"}
            </span>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-xs mt-1" style={{ color: MUTED }}>
                Click any student on the left to view their dashboard
              </p>
            </div>
          </div>
        ) : (
                ...
                <>
                  {activeTab === "schedule" && (
                    <div className="max-w-lg space-y-6">
                      ...
                      <div>
                        ...
                        >
                          {JAM_SUBJECTS.map((s) => {
                            const speed =
                              (
                                studentData.topicSpeed as Record<string, string>
                              )[s.id] ?? "first_normal";
                            return (
                              <div
                                key={s.id}
                              ...
                              >
                                ...
                                <select
                                  ...
                                  onChange={async (e) => {
                                    if (!selected) return;
                                    const next = {
                                      ...(studentData.topicSpeed as Record<
                                        string,
                                        string
                                      >),
                                      [s.id]: e.target.value,
                                ...
                                >
                                  <option value="first_slow">
                                    1st time — Slow (×1.3)
                                  </option>
                                  <option value="first_normal">
                                    1st time — Normal (×1.0)
                                  </option>
                                  <option value="first_fast">
                                    1st time — Fast (×0.8)
                                  </option>
                                  <option value="second_slow">
                                    2nd time — Slow (×1.0)
                                  </option>
                                  <option value="second_normal">
                                    2nd time — Normal (×0.67)
                                  </option>
                                  <option value="second_fast">
                                    2nd time — Fast (×0.5)
                                  </option>
                                </select>
                              </div>
                      ...
                      <div>
                        ...
                        >
                          {(studentData.subjectOrder.length > 0
                            ? studentData.subjectOrder
                            : JAM_SUBJECTS.map((s) => s.id)
                          ).map((id, idx) => {
                            const subject = JAM_SUBJECTS.find(
                              (s) => s.id === id,
                            );
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex gap-1">
                                  ...
                                  <button
                                    type="button"
                                    disabled={idx === JAM_SUBJECTS.length - 1}
                                    onClick={async () => {
                                      if (!selected) return;
                                      const order =
                                        studentData.subjectOrder.length > 0
                  ...
                  {activeTab === "syllabus" && (
                    <div className="max-w-lg">
                      ...
                      >
                        <SyllabusPanel
                          data={studentData.syllabus as Record<string, unknown>}
                        />
                      </div>
                    </div>
                  )}
