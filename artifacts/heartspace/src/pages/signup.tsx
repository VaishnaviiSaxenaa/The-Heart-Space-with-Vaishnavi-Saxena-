...
import { supabase, ROLE_MAP, SERVICE_INFO, type SupabaseRole } from "../lib/supabase";
...
const SERVICES: Service[] = [
  {
    ...
    bullets: ["1-on-1 counsellor sessions", "Syllabus & assignment tracking", "Mood & wellness monitoring", "AI progress summaries"],
  },
  {
    key:     "prep_student",
    name:    "Apex+",
    ...
    bullets: ["Syllabus & assignment tracker", "Daily tracker & analytics", "AI study guidance", "Self-paced progress"],
  },
  {
    ...
    bullets: ["Dedicated counsellor sessions", "Emotional wellness tracking", "Mood & daily journals", "Safe space support"],
  },
];
...
/* ── Service selector ──────────────────────────────────────── */
function ServiceSelector({ onSelect }: { onSelect: (key: ServiceKey) => void }) {
  ...
  return (
    <div className="w-full max-w-3xl z-10">
      ...
      <div className="text-center mb-6">
        <h2 className="font-serif text-2xl font-bold" style={{ color: SIDEBAR }}>Choose your plan</h2>
        <p className="text-sm mt-1" style={{ color: MUTED }}>Select the service that fits your journey</p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h3 className="font-serif text-xl font-bold leading-none" style={{ color: s.color }}>{s.name}</h3>
                  <p className="text-[11px] mt-0.5" style={{ color: MUTED }}>{s.tagline}</p>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-2 mb-5">
                {s.bullets.map((b) => (
                  <div key={b} className="flex items-start gap-2">
                    <div className="mt-1 w-3 h-3 rounded-full flex-shrink-0" style={{ background: `${s.color}44` }}>
                      <div className="w-full h-full rounded-full scale-50 flex items-center justify-center">
                        <div className="w-1.5 h-1.5 rounded-full" style={{ background: s.color }} />
                      </div>
                    </div>
                    <span className="text-xs leading-relaxed" style={{ color: CHARCOAL }}>{b}</span>
                  </div>
                ))}
              </div>
      ...
      <p className="text-center text-sm mt-6" style={{ color: MUTED }}>
        ...
        <a href="/" className="font-semibold underline underline-offset-2" style={{ color: GOLD }}>Sign in</a>
      </p>
    </div>
  );
}
  ...
  async function onSubmit(v: SignupFormValues) {
    ...
    try {
      ...
      if (signInError || !signInData?.session) {
        /* Email confirm gate — log in locally with selected service role */
        ...
        toast({ title: `Welcome, ${v.fullName.split(" ")[0]}! 🎉`, description: "Your account is ready. Dive in!" });
      ...
      toast({ title: `Welcome to ${service.name}! 🎉`, description: `Hi ${v.fullName.split(" ")[0]}, your account is ready.` });
      setLocation(mapped.redirect);
    } catch (err: any) {
      toast({ title: "Sign up failed", description: err?.message ?? "Something went wrong. Please try again.", variant: "destructive" });
    } finally {
      setIsPending(false);
    }
  }
  ...
  return (
    <div className="w-full max-w-[420px] z-10">
      ...
      <div className="rounded-2xl p-8"
        ...
        <div className="flex items-center gap-2 mb-5 px-3 py-2 rounded-xl" style={{ background: service.tagBg }}>
          <span className="text-lg">{service.emoji}</span>
          <div>
            <p className="text-xs font-semibold" style={{ color: service.color }}>{service.name}</p>
            <p className="text-[10px]" style={{ color: MUTED }}>{service.tagline}</p>
          </div>
          <button
            onClick={onBack}
        ...
        <p className="text-sm mb-6" style={{ color: MUTED }}>You'll be inside immediately — no confirmation needed</p>
...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden" style={bgStyle}>
