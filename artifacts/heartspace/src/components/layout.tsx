...
import {
  ...
  Brain, DollarSign, Library, FileText, BarChart2, LogOut, Menu, X, Sparkles,
} from "lucide-react";
...
const GOLD     = "#E6A756";
const CARD     = "#F3EDE6";
const SIDEBAR  = "#EBE3D9";    /* slightly darker than main bg */
const SIDEBAR_HEADER = "#5C3D2E"; /* deep warm brown */
const MUTED    = "#8C7B70";
const BORDER   = "#D8CFC4";
...
const NAV_ITEMS = [
  ...
  { icon: FlaskConical,     label: "PhD Journey",     key: "phd",        soon: true },
  ...
  { icon: DollarSign,       label: "Finances",        key: "finance",    soon: true },
  { icon: Library,          label: "Resources",       key: "resources",  soon: true },
  { icon: FileText,         label: "Notes",           key: "notes",      soon: true },
  { icon: BarChart2,        label: "Reports",         key: "reports",    soon: true },
];
    ...
    >
      ...
      <div className="px-6 pt-7 pb-6" style={{ borderBottom: `1px solid ${BORDER}` }}>
        <div className="flex items-start justify-between">
          ...
          {onClose && (
            <button onClick={onClose} className="md:hidden p-1" style={{ color: MUTED }}>
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
      ...
      <nav className="flex-1 px-3 py-5 space-y-0.5">
        {NAV_ITEMS.map(({ icon: Icon, label, key, href, soon }) => {
          const dest = href ?? homePath;
          const active = soon ? false : (key === "home" ? (location === "/dashboard" || location === "/counsellor") : location === dest);
          return (
            <div key={key}>
              {soon ? (
                <div
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-default opacity-50"
      ...
      <div className="px-3 pb-5 space-y-3">
        ...
        <div className="rounded-2xl p-4" style={{ background: `${SIDEBAR_HEADER}14`, border: `1px solid ${BORDER}` }}>
          ...
          <p className="text-xs font-serif italic leading-relaxed" style={{ color: CHARCOAL }}>
        ...
        <div className="rounded-2xl p-4" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
          ...
          <div className="mt-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs capitalize" style={{ color: GOLD }}>{user?.role}</p>
            </div>
